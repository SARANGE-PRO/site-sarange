...
const ProductConfigurator = ({ isOpen, onClose, productType, onAdd }) => {
  ...
  const renderColorSection = () => {
    ...
    const colorsPVC = [
      ...
      {
        label: 'Gris 7016 Ext / Blanc Int',
        value: 'Bicolore Gris 7016',
        hex: 'linear-gradient(135deg, #374151 50%, #ffffff 50%)',
        border: 'border-transparent',
      },
      {
        label: 'Chêne Doré Ext / Blanc Int',
        value: 'Bicolore Chêne Doré',
        hex: 'linear-gradient(135deg, #B45F06 50%, #ffffff 50%)',
        border: 'border-transparent',
      },
    ];
    ...
    const colorsMixte = [
      ...
      {
        label: 'Chêne Doré Ext / Blanc Int',
        value: 'Bicolore Bicolore Chêne Doré',
        hex: 'linear-gradient(135deg, #B45F06 50%, #ffffff 50%)',
        border: 'border-transparent',
      },
    ];
      ...
      <ConfigSection title="🎨 Couleur & Finition">
        <div className="flex flex-wrap gap-3">
          {activeColors.map((c) => (
            ...
            >
              <div
                className="w-6 h-6 rounded-full mr-2 shadow-sm border border-black/10"
      ...
      const types = [
        {
          ...
          icon: <WindowIcons.OneLeaf selected={data.type === 'Fenêtre 1 vantail'} />,
        },
        {
          label: 'Fenêtre 2 vantaux',
          icon: <WindowIcons.TwoLeaves selected={data.type === 'Fenêtre 2 vantaux'} />,
        },
        {
          label: 'Porte-Fenêtre',
          icon: <WindowIcons.FrenchDoor selected={data.type === 'Porte-Fenêtre'} />,
        },
        {
          label: 'Châssis Fixe',
          icon: <WindowIcons.Fixed selected={data.type === 'Châssis Fixe'} />,
        },
        {
          label: 'Fenêtre coulissante (PVC)',
          icon: <WindowIcons.Sliding selected={data.type === 'Fenêtre coulissante (PVC)'} />,
        },
        {
          label: '3 vantaux ou composé',
          icon: <WindowIcons.Composite selected={data.type === '3 vantaux ou composé'} />,
        },
      ];
          ...
          <ConfigSection title="2️⃣ Type d'ouverture">
            <div className="grid grid-cols-2 gap-3">
              {types.map((item) => (
                <button
                  key={item.label}
                  onClick={() => setData({ ...data, type: item.label })}
                ...
                >
                  ...
                  <span
                    className={`text-xs font-bold text-center ${data.type === item.label ? 'text-orange-700' : 'text-slate-600'}`}
                  >
                    {item.label}
                  </span>
                </button>
          ...
          <div className="mb-6 space-y-3">
                ...
                <input
                  ...
                  onChange={(e) => {
                    const newOpts = e.target.checked
                      ? [...data.options, 'Oscillo-battant']
                      : data.options.filter((o) => o !== 'Oscillo-battant');
            ...
            {data.type && data.type !== 'Châssis Fixe' && (
              <label className="flex items-center space-x-2 cursor-pointer bg-slate-50 p-3 rounded-lg border border-slate-200">
                <input
                  type="checkbox"
                  className="w-5 h-5 rounded text-blue-500 accent-blue-500"
                  onChange={(e) => {
                    const newOpts = e.target.checked
                      ? [...data.options, 'Grille de ventilation']
                      : data.options.filter((o) => o !== 'Grille de ventilation');
                    setData({ ...data, options: newOpts });
                  }}
                />
                <span className="text-sm font-bold text-slate-700">
                  Option Grille de ventilation (VMC)
                </span>
              </label>
            )}
          </div>
      ...
      return (
        <ConfigSection title="2️⃣ Modèle & Matériau">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
            <button
              onClick={() => setData({ ...data, material: 'Mixte PVC/Alu', stylePanel: '' })}
            ...
            <button
              onClick={() => setData({ ...data, material: 'Aluminium', stylePanel: '' })}
            ...
            <button
              onClick={() =>
                setData({
                  ...data,
                  material: 'Porte de service PVC',
                  stylePanel: 'Panneau sandwich 28mm',
                })
              }
              ...
              <label className="flex items-center space-x-2 cursor-pointer mb-4">
                <input
                  ...
                  onChange={(e) => {
                    const newOpts = e.target.checked
                      ? [...data.options, 'Tierce']
                      : data.options.filter((o) => o !== 'Tierce');
                    setData({ ...data, options: newOpts });
                  }}
                />
                <span className="text-sm font-medium text-slate-700">
                  Ajouter une Tierce (Côté vitré ou plein)
                </span>
              </label>
              <div>
      ...
      return (
        <ConfigSection title="2️⃣ Configuration Volet">
          <div className="grid grid-cols-1 gap-2 mb-4">
            {[
              'Rénovation (Coffre extérieur)',
              'Traditionnel (Dans coffre existant)',
              'Monobloc (Avec fenêtre)',
            ].map((t) => (
              <button
                key={t}
                onClick={() => setData({ ...data, type: t })}
          ...
          {(data.options.includes('Moteur Radio') || data.options.includes('Moteur Solaire')) && (
            <label className="flex items-center space-x-2 cursor-pointer bg-blue-50 p-3 rounded-lg border border-blue-100">
              <input
                ...
                onChange={(e) => {
                  const newOpts = e.target.checked
                    ? [...data.options, 'Domotique Connectée']
                    : data.options.filter((o) => o !== 'Domotique Connectée');
                  setData({ ...data, options: newOpts });
                }}
              />
        ...
        <ConfigSection title="2️⃣ Type de Porte">
          ...
          <label className="flex items-center space-x-2 cursor-pointer bg-slate-50 p-3 rounded-lg border">
            <input
              ...
              onChange={(e) => {
                const newOpts = e.target.checked
                  ? [...data.options, 'Portillon Intégré']
                  : data.options.filter((o) => o !== 'Portillon Intégré');
                setData({ ...data, options: newOpts });
              }}
            />
            <span className="text-sm font-medium text-slate-700">
              Option Portillon intégré (Porte piéton)
            </span>
          </label>
        </ConfigSection>
      );
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <button
                  key={sub}
                  onClick={() => setData({ ...data, subtype: sub })}
            ...
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-4">
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-2">
                  Référence VELUX ou Dimensions
                </label>
                <input
                  type="text"
                  placeholder="Ex: MK04 ou 78x98cm"
              ...
              <label className="flex items-center space-x-2 cursor-pointer bg-white p-3 rounded-lg border">
                <input
                  ...
                  onChange={(e) => {
                    const newOpts = e.target.checked
                      ? [...data.options, 'Avec Store']
                      : data.options.filter((o) => o !== 'Avec Store');
                    setData({ ...data, options: newOpts });
                  }}
                />
                <span className="text-sm font-medium text-slate-700">
                  Ajouter un Store Solaire / Occultant ?
                </span>
              </label>
            </div>
          )}
              ...
              <div className="bg-orange-50 p-4 rounded-xl border border-orange-100 space-y-4">
                <div className="flex justify-between items-center">
                  <p className="text-sm text-orange-800 font-bold flex items-center">
                    <Info size={16} className="mr-2" /> Mesures spécifiques
                  </p>
                  <button
                    onClick={() => {
                      setGuideType('cintrage');
                      setShowGuide(true);
                    }}
                    className="text-xs text-blue-600 underline font-bold flex items-center"
                  >
                    <Info size={14} className="mr-1" /> Aide mesure flèche
                  </button>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      type="number"
                      className="w-full p-3 border border-orange-200 rounded-lg bg-white"
                      value={data.width}
                  ...
                  <div className="col-span-2">
                    ...
                    <input
                      ...
                      className="w-full p-3 border border-orange-200 rounded-lg bg-white"
                      value={data.fleche}
                      onChange={(e) => setData({ ...data, fleche: e.target.value })}
                    />
                  </div>
                </div>
              </div>
  ...
  const handleAdd = () => {
    if (
      ...
      (data.subtype && data.subtype !== 'Fenêtre de toit (VELUX)')
    ) {
      ...
      >
        ...
        >
          ...
          <div className="p-6 overflow-y-auto">
            <ConfigSection title="1️⃣ Quantité">
              <div className="flex items-center">
                <button
                  onClick={() => setData((d) => ({ ...d, quantity: Math.max(1, d.quantity - 1) }))}
                ...
                <button
                  onClick={() => setData((d) => ({ ...d, quantity: d.quantity + 1 }))}
                  className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center font-bold text-slate-600 hover:bg-slate-200"
                >
                  +
                </button>
                ...
                <div className="flex items-center bg-slate-100 rounded-xl p-2 border border-slate-200">
                  <div className="flex-1 px-4 border-r border-slate-300">
                    ...
                    <input
                      type="number"
                      placeholder="1200"
                      className="bg-transparent font-mono text-xl font-bold w-full outline-none"
                      value={data.width}
                      onChange={(e) => setData({ ...data, width: e.target.value })}
                    />
                  </div>
              ...
              <div className="flex flex-col gap-2">
                <label className="flex items-center p-3 border rounded-lg cursor-pointer hover:bg-slate-50 transition-colors group">
                  <input
                    type="radio"
                    name="pose"
                    checked={data.pose === 'Fourniture + Pose SARANGE'}
                    onChange={() => setData({ ...data, pose: 'Fourniture + Pose SARANGE' })}
                    className="text-orange-500 w-5 h-5 accent-orange-500"
                  />
                  <span className="ml-3 text-sm font-bold text-slate-700 group-hover:text-orange-600">
                    Fourniture + Pose (Recommandé)
                  </span>
                  <span className="ml-auto text-xs bg-green-100 text-green-700 px-2 py-1 rounded font-bold">
                    TVA 5.5%
                  </span>
                </label>
                <label className="flex items-center p-3 border rounded-lg cursor-pointer hover:bg-slate-50 transition-colors">
                  <input
                    type="radio"
                    name="pose"
                    checked={data.pose === 'Fourniture seule'}
                    onChange={() => setData({ ...data, pose: 'Fourniture seule' })}
                    className="text-orange-500 w-5 h-5 accent-orange-500"
                  />
                  <span className="ml-3 text-sm font-medium text-slate-600">
                    Fourniture seule (Livraison B2B/Chantier)
                  </span>
                </label>
              </div>
            </div>
          ...
          <div className="p-4 border-t bg-slate-50 shrink-0">
            <button
              onClick={handleAdd}
              className="w-full py-3 bg-slate-900 text-white font-bold rounded-xl hover:bg-slate-800 transition-colors shadow-lg flex items-center justify-center"
            >
